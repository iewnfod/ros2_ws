...
set_gripper_command::set_gripper_command(bool target) noexcept
    : command("set_gripper_command")
	, target_(target)
{
    gripper_subsystem_ =
...
void set_gripper_command::initialize() noexcept
{
	gripper_subsystem_->set_gripper(target_);
    TROLLY_INFO("set_gripper_command set to %d.", target_);
    // call gripper_subsystem_'s methods to perform desired actions...
}

void set_gripper_command::execute() noexcept {}

void set_gripper_command::end(bool /* interrupted */) noexcept { TROLLY_INFO("set_gripper_command end."); }

bool set_gripper_command::is_finished() noexcept { return true; }
