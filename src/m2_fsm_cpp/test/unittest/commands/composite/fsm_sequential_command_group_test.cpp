...
TROLLY_TEST(sequential_command_group_test, multiple_commands_test)
{
    ...
    auto scg_id = cs.schedule_command(
        std::make_unique<sequential_command_group>(std::move(cmd1), std::move(cmd2)));
    TROLLY_EXPECT_NE(scg_id, COMMAND_INSTANCE_ID_INVALID);
    auto* scg = dynamic_cast<sequential_command_group*>(cs.get_command(scg_id));
